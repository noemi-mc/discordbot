...
client.on('interactionCreate', async interaction => {
    ...
    const parsedFields = interaction.customId.split("_");
    const row = parseInt(parsedFields[1], 10);
    const col = parseInt(parsedFields[2], 10);
    ...
    if (isGameOver()) {
        ...
        await user.increment('score');
         ...
         interaction.update({
            content:"You won the game" + (user.get('score') + 1) + "time(s).",
            components:[]
         })
